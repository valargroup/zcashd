...
/// - V1 for epochs before NU5 (ZIP 221 as originally deployed at Heartwood);
/// - V2 for NU5 through NU6.2 (adds the Orchard fields);
/// - V3 for NU6.3 onward (adds the Ironwood fields).
///
/// The ZFUTURE test-only epoch (branch id 0xffffffff) is not a `BranchId`, and follows the
/// latest deployed format.
fn dispatch<S, T>(
    cbranch: u32,
    input: S,
    v1: impl FnOnce(S) -> T,
