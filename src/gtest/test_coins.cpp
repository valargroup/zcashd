...
public:
    ...
    TxWithNullifiers()
    {
        ...
        ironwoodBuilder.AddOutput(std::nullopt, to, 0, std::nullopt);
        mutableTxV5.ironwoodBundle = ironwoodBuilder.Build().value().ProveAndSign({}, dataToBeSigned).value();
        ironwoodNullifier = mutableTxV5.ironwoodBundle.GetNullifiers().at(0);
...
void checkNullifierCache(const CCoinsViewCacheTest &cache, const TxWithNullifiers &txWithNullifiers, bool shouldBeInCache) {
    ...
    EXPECT_FALSE(cache.GetNullifier(txWithNullifiers.ironwoodNullifier, SPROUT));
    EXPECT_FALSE(cache.GetNullifier(txWithNullifiers.ironwoodNullifier, SAPLING));
    EXPECT_FALSE(cache.GetNullifier(txWithNullifiers.ironwoodNullifier, ORCHARD));
    ...
    EXPECT_EQ(containsIronwoodNullifier, shouldBeInCache);
}
...
TEST(CoinsTests, SubtreesTest)
{
    ...
    {
    SCOPED_TRACE("Ironwood");
    testSubtreesForShieldedType(IRONWOOD);
    }
}
