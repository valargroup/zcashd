...
public:
    ...
    // Adds the tree to the corresponding shielded anchor cache
    ...
    // Orchard, and Ironwood are supported.
    void PushSubtree(ShieldedType type, libzcash::SubtreeData subtree);

    // Pop a subtree out of the database. Only Sapling, Orchard, and Ironwood
    // are supported. Throws an exception if there isn't a subtree present
    // in the database.
